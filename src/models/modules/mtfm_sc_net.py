...
class SCNet(nn.Module):
    def __init__(self, hparams: dict):
        ...
        self.sc_convs = nn.ModuleList([
            ...
            for window_size in hparams['conv_kernel_size_tuple']
        ])
        self.sc_fcl = nn.Linear(
            in_features=hparams['conv_num_kernel'] * len(hparams['conv_kernel_size_tuple']),
            out_features=hparams['num_api'],
        )
        self.fic_fc = nn.Linear(
            in_features=hparams['conv_num_kernel'] * len(hparams['conv_kernel_size_tuple']),
            out_features=hparams['feature_dim']
        )
        self.fic_api_feature_embedding = nn.Parameter(torch.rand(hparams['feature_dim'], hparams['num_api']))
        self.fic_mlp = nn.Sequential(
            nn.Linear(in_features=hparams['feature_dim'] * 2, out_features=hparams['feature_dim']),
            nn.Linear(in_features=hparams['feature_dim'], out_features=1),
            nn.Tanh()
        )
    ...
    def forward(self, x):
        """
        :param x: (batch_size, text_len, embedding_size)
        :return (batch_size, num_api)
        """
        # semantic component
        x_trans = torch.transpose(x, 1, 2)  # (batch_size, embedding_size, text_len)
        e = [conv(x_trans) for conv in self.sc_convs]
        e = torch.cat(e, dim=2)  # (batch_size, conv_num_kernel, len(conv_kernel_size_tuple))
        e = e.view(e.size(0), -1)  # (batch_size, conv_num_kernel * len(conv_kernel_size_tuple))
